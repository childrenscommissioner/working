...
**% of respondents overall saying they are unhappy with an aspect of their life that think its unlikely they will have a better life than their parents**
...
corr_list<-rbindlist(lapply(hap_cols,function(x){
  ...
  tb<-melt(df_imp[,..col
                              ...
                              ][!is.na(get(x))][,value:=ifelse(value %in% target_vals,1,0)][,.(c=sum(wt)),by=.(hap=get(x),variable,value)
...
corr_list<-rbindlist(lapply(hap_cols,function(x){
  ...
  tb<-melt(df_imp[,..col
                                           ...
                                           ifelse(get(x) %in% c("Unhappy","Very unhappy","sad"),1,0))
                              ][!is.na(get(x))][,value:=ifelse(value %in% target_vals,1,0)
