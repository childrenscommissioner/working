...
```{r}
hap_col<-names(df)[grepl(paste0("^id$|UTLA19CD|ward|^wt$|",search_str),names(df))]
...
names(tb_count)[!grepl("UTLA",names(tb_count))]<-gsub(" |[']","_",names(tb_count)[!grepl("UTLA",names(tb_count))])
...
pal2<-colorNumeric("Reds",c(0,max(df_la$perc,na.rm=T)*100))

leaf_poly@data<-merge(leaf_poly@data,tb_map,by.x="ctyua19cd",by.y="UTLA19CD",all.x=T)

leaf_poly@data<-merge(leaf_poly@data,tb_count,by.x="ctyua19cd",by.y="UTLA19CD",all.x=T)
...
map %>%
  ...
  addLegend("topleft", pal = pal2, values = seq(0,max(df_la$perc,na.rm=T)*100),bins=5,
...
labels<-lapply(opt_var,function(x){
  ...
  paste0("Constituency: ",leaf_poly@data$pcon19nm,paste0("</br>Measure: ",col1),lb,"</br>% of constituency cohort: ",round(leaf_poly@data[,x]),"%")
